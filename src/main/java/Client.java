...
public class Client {
    ...
    private static List<Integer> checkingIfIdIsTrulyUnique = new ArrayList<>();
    ...
    public static long createId() {
        ...
        if (!checkingIfIdIsTrulyUnique.contains(checkedId)){ // sprawdzenie czy wylosowanego id nie ma juz wczesniej dodanego do listy
            checkingIfIdIsTrulyUnique.add(checkedId);        // jesli nie ma to go dodaj, aby lista byla kompletna przy nastepnym tworzeniu nowego id
            return checkedId;
    ...
    public void setListOfClientAccounts(List<Account> listOfAccounts) {
    ...
    public static class ClientBuilder {
        ...
        public ClientBuilder addNewListOfAccounts(String typeOfAccounts) { // metoda dodająca konta przy tworzeniu klienta pracujaca na ClientBuilderze (jedno kont na poczatek mozna utworzyc). Potem nastepme ,pzma dpdawac druga metoda
            listOfClientAccounts = new ArrayList<>();

                System.out.println("Podaj saldo początkowe dla konta " + typeOfAccounts);
                Scanner scanner = new Scanner(System.in);
                double balance = scanner.nextDouble();
                Account account = Account.createAccount(typeOfAccounts, balance); //stworzenie konta
    ...
    public Client addOtherAccountToList(String accountType, double balance) { // dodanie nowego konta juz po utworzeniu klienta (w trakcie). Pracujemy juz na kliencie nie na klientBuilderze
        ...
        if (accountType.equalsIgnoreCase(AccountType.STUDENT.name())) {
            account1 = Account.createAccount(AccountType.STUDENT.name(), balance);
            ...
            account1 = Account.createAccount(AccountType.PRO.name(), balance);
            this.getListOfClientAccounts().add(account1);
            Bank.allAccounts.add(account1); // dodanie na listę wszystkich kont w calym banku ( w kazdym miejscu gdzie tworzymy konto, dodajemy je do ogolnej listy kont)
            System.out.println("Operacja dodania nowego konta przebiegła pomyślnie.");
            return this;
            ...
            account1 = Account.createAccount(AccountType.STANDARD.name(), balance);
            this.getListOfClientAccounts().add(account1);
            Bank.allAccounts.add(account1); // dodanie na listę wszystkich kont w calym banku ( w kazdym miejscu gdzie tworzymy konto, dodajemy je do ogolnej listy kont)
            System.out.println("Operacja dodania nowego konta przebiegła pomyślnie.");
            return this;
    ...
    public static Client createClient(String name, String surname, long pesel, String accountType) { // tworzenie nowego klienta przez metodę aby w main nie tworzyc Client client = new Client...
    ...
    public Client cashAdd() {
        ...
        if (this.getListOfClientAccounts().stream()  // sprawdzamy czy to co wpisal uzytkownik jest zgodne z jego typami ktore posiada
                .filter(account -> account.getAccountType().equalsIgnoreCase(choosenAccountType))
                .toArray().length>0) {
            ...
            this.getListOfClientAccounts().stream() //zmiana salda
                    .filter(account -> account.getAccountType().equalsIgnoreCase(choosenAccountType)) // znajdz konto ktore wpisal uzytkownik
